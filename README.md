# Alan Delgado Portfolio

A dependency-free, responsive portfolio prepared for GitHub Pages.

## Publish as the main GitHub Pages site

1. Create a public repository named `delgadoliveira.github.io`.
2. Copy `index.html` to the repository root.
3. Commit and push to the `main` branch.
4. In **Settings → Pages**, select **Deploy from a branch**, then choose `main` and `/ (root)`.
5. The site will be available at `https://delgadoliveira.github.io`.

## Preview locally

Open `index.html` in a browser. No build step or dependencies are required.

## Content sources

- GitHub profile and public repositories: `github.com/delgadoliveira`
- Research identity: ORCID `0000-0003-4260-5844`
- AI agents course: `delgadoliveira/topicos-especiais-ia`

Before publishing, review the professional summary and location language to ensure it matches
the level of detail you want to share publicly.
